...
public class ControladorJoc {
	...
	public PartidaEnJoc carregarPartida(Partida p) throws ExcepcioPartida {
		...
		if (this.partidaCarregada.getInfoPartida().getRol() == Rol.CODEMAKER) {
			Integer algoritme = 0;
			if (this.partidaCarregada.getInfoPartida().getDificultat() == Dificultat.DIFICIL) algoritme = 1;
			this.codemaker = new Codemaker();
			this.codebreaker = new CodebreakerMaquina(this.partidaCarregada.getInfoPartida(), algoritme);
		}
		else {
			this.codemaker = new Codemaker();
			this.codebreaker = new Codebreaker(this.partidaCarregada.getInfoPartida());
			return tornCodemakerMaquina();
		}
	...
	private Boolean codiDificultatValid(List<Integer> codi, Dificultat dificultat) {
		for (int i = 0; i < 4; ++i) {
			...
			if (dificultat == Dificultat.FACIL) {
				...
				if (codi.indexOf(color) != codi.lastIndexOf(color)) return false;
			}
	...
	public PartidaEnJoc tornCodemakerHuma(List<Integer> codiSecret) throws Exception {
		...
		else throw new ExcepcioPartida("No es possible jugar amb aquesta combinacio de colors. Si us plau, torna-la a introduir.");
	...
	/**
	 * Comprova paramteres i realiza un intent per a desxifrar el codi secret per part de l'usuari
	...
	public PartidaEnJoc tornCodebreakerHuma(List<Integer> codi) throws ExcepcioPartida {
		...
		List<Integer> solucio = this.partidaCarregada.getInfoPartida().getCodiSecret();

		this.partidaCarregada.intentarCodi(codi);
		
		Pair<Integer, Integer> correccio = this.codemaker.validarCodi(codi, solucio);
	...
	public PartidaEnJoc infoPartidaEnJoc() throws ExcepcioPartida {
		...
		if (this.partidaCarregada.getNumTorn() > 0) {
			if (this.partidaCarregada.getInfoPartida().getRol() == Rol.CODEBREAKER) this.partidaCarregada.restaPuntuacio(-100);
			else this.partidaCarregada.restaPuntuacio(100);
		}
